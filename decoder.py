...
class DECODER():
    def __init__(self,args, sess, model_type, mode=0):
        self.args = args
        self.sess = sess
        # Get trained model and load
        if model_type == 'WAVENET':
            self.model = Wavenet_Model(self.args, self.sess)
        elif model_type == 'RNN':
            self.model = RNN_Model(self.args, self.sess)
...
#        test_set = SpeechLoader(self.args.test_data_dir, self.args.num_features, self.args.num_classes)
#        test_wave = test_set.mel_freq
#        test_lbl = test_set.target_label
        test_wave = np.load(os.path.join(self.args.test_data_dir, 'waves_0.npy'), encoding='bytes')
        test_lbl = np.load(os.path.join(self.args.test_data_dir, 'trans_0.npy'), encoding='bytes')
        test_wave_padded, test_wave_length = pad_sequences(test_wave[10:20])
        test_sparse_lbl = sparse_tensor_form(test_lbl[10:20])
        ...
        np.save(self.ctc_file, char_prob)
